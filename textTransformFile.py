...
encode_type = argv[1]
file = argv[2] 
if encode_type == 'help' and file == 'help':  	print 'Options: hex, binary, rot18, reverse, caesar[numb], vigenere[key]\n ONLY ACCEPTS ".txt" documents'; quit();
f = open(file,"r+")
inp = f.read().split('\n')
f.close()
...
if 	 encode_type 	 == 'hex': 		
	...
	for x in range(len(inp)): r.write(encode_hex(inp[x])+'\n')
	r.close()
	print 'encoded'
elif encode_type 	 == 'binary':
	r = open(file[:-4]+'_'+encode_type+'.txt','w+'); 
	for x in range(len(inp)): r.write(encode_binary(inp[x])+'\n')
	r.close()
	print 'encoded'			
elif encode_type	 == 'rot18':
	r = open(file[:-4]+'_'+encode_type+'.txt','w+'); 
	for x in range(len(inp)): r.write(rot18(inp[x])+'\n')
	r.close()
	print 'encoded'								
elif encode_type	 == 'reverse':
	r = open(file[:-4]+'_'+encode_type+'.txt','w+'); 
	for x in range(len(inp)): r.write(inp[x][::-1]+'\n')
	r.close()
	print 'encoded'							
elif encode_type[:6] == 'caesar':
	r = open(file[:-4]+'_'+encode_type+'.txt','w+'); 
	for x in range(len(inp)): r.write(caesar(inp[x]+'\n',int(encode_type[6:])))
	r.close()
	print 'encoded'		
elif encode_type[:8] == 'vigenere':
	r = open(file[:-4]+'_'+encode_type+'.txt','w+'); 
	for x in range(len(inp)): r.write(vigenere(inp[x]+'\n',encode_type[8:]))
	r.close()
	print 'encoded'		
